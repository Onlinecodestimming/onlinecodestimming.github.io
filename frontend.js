...
let tracks=[],current=-1,featured=null,shuffleOn=false,repeatOn=false;
const clean=value=>String(value||"Untitled").replace(/\.[^/.]+$/,""),art=t=>t.artwork||fallback;
const saved=()=>{try{return JSON.parse(localStorage.getItem("musicfy-favorites")||"[]")}catch{return[]}},isFavorite=t=>saved().includes(String(t.id)),toggleFavorite=t=>{const ids=saved(),id=String(t.id),next=ids.includes(id)?ids.filter(x=>x!==id):[...ids,id];localStorage.setItem("musicfy-favorites",JSON.stringify(next));renderAll();updateFavoriteButton()};

function card(track,{artist=false}={}){
  const el=document.createElement("article");
  el.className="card"+(artist?" artist-card":"");
  el.innerHTML=`<img alt=""><div class="title"></div><div class="sub"></div>`;
  el.querySelector("img").src=art(track);
  el.querySelector(".title").textContent=clean(artist?track.artist:track.title);
  el.querySelector(".sub").textContent=artist?"Artist":track.artist||"Unknown artist";
  el.onclick=()=>artist?showArtist(track.artist):play(track);
  return el
}
...
function openContextMenu(event, track) {
  ...
  menu.querySelector("#cm-play-next").onclick = () => {
    tracks.splice(current + 1, 0, track);
    menu.remove();
  };
...
function fill(id,items,fn){const box=$(id);box.replaceChildren();if(!items.length){box.innerHTML='<p class="empty">Nothing here yet.</p>';return}items.forEach(x=>box.append(fn(x)))}
...
function renderAll(){
  renderHome();
  fill("libraryList",tracks,trackRow);
  ...
  fill("favoritesList",tracks.filter(isFavorite),trackRow);
  
  const playlistsContainer = $("playlistsPage");
  if(playlistsContainer) playlistsContainer.innerHTML = '<p class="empty">No playlists created yet.</p>';
}
...
function showAlbum(name){go(`album/${encodeURIComponent(name)}`)};function showArtist(name){go(`artist/${encodeURIComponent(name)}`)}
...
function applyRoute(){
  ...
  if(kind==="artist"||kind==="album"){
    ...
    fill("detailList",matches,trackRow);
    showPage("detail");
    return
  }
...
function play(track){
  current=tracks.findIndex(t=>String(t.id)===String(track.id));
  if(current<0)return;
  audio.src=track.url;
  audio.play().catch(()=>{});
...
function updateFavoriteButton(){const t=tracks[current],b=$("favorite");b.textContent=t&&isFavorite(t)?"♥":"♡";b.classList.toggle("on",Boolean(t&&isFavorite(t)))}
function next(delta){if(!tracks.length)return;if(shuffleOn&&delta>0){let pick=Math.floor(Math.random()*tracks.length);if(tracks.length>1)while(pick===current)pick=Math.floor(Math.random()*tracks.length);current=pick}else current=(current+delta+tracks.length)%tracks.length;play(tracks[current])}
$("play").onclick=()=>{if(!tracks.length)return;if(current<0)play(featured||tracks[0]);else if(audio.paused)audio.play().then(()=>$("play").textContent="❚❚");else audio.pause()};$("previous").onclick=()=>next(-1);$("next").onclick=()=>next(1);$("shuffle").onclick=()=>{shuffleOn=!shuffleOn;$("shuffle").classList.toggle("on",shuffleOn)};$("repeat").onclick=()=>{repeatOn=!repeatOn;$("repeat").classList.toggle("on",repeatOn)};$("volume").oninput=e=>audio.volume=e.target.value/100;$("heroPlay").onclick=()=>featured&&play(featured);$("favorite").onclick=()=>tracks[current]&&toggleFavorite(tracks[current]);audio.onpause=()=>$("play").textContent="▶";audio.onplay=()=>$("play").textContent="❚❚";audio.onended=()=>repeatOn?audio.play():next(1);audio.ontimeupdate=()=>{if(audio.duration){$("progress").value=audio.currentTime/audio.duration*100;$("currentTime").textContent=format(audio.currentTime)}};audio.onloadedmetadata=()=>$("duration").textContent=format(audio.duration);$("progress").oninput=e=>{if(audio.duration)audio.currentTime=audio.duration*e.target.value/100};function format(s){if(!Number.isFinite(s))return"0:00";return `${Math.floor(s/60)}:${String(Math.floor(s%60)).padStart(2,"0")}`}
document.querySelectorAll(".nav").forEach(b=>b.onclick=()=>b.dataset.action==="search"?$("search").focus():go(b.dataset.page));$("search").oninput=e=>{const q=e.target.value.toLowerCase().trim();fill("libraryList",tracks.filter(t=>[t.title,t.artist,t.album].some(v=>String(v||"").toLowerCase().includes(q))),trackRow);go("library")};
